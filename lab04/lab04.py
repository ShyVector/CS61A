...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(affordable_fruits, amount, cart):
        if affordable_fruits == [] and amount == 0:
            print(cart)
        elif affordable_fruits and amount > 0:
            fruit_to_buy = affordable_fruits[0]
            fruit_price = prices[fruit_to_buy]
            for k in range(1, amount // fruit_price + 1):
                # Hint: The display function will help you add fruit to the cart.
                add(
                    affordable_fruits[1:],
                    amount - fruit_price * k,
                    cart + display(fruit_to_buy, k),
                )
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    targ = make_city("Target", lat, lon)
    return (
        get_name(city_a)
        if distance(city_a, targ) < distance(city_b, targ)
